...
#define OLDCODE_FOR_TRIANGLEMESH
...
RT_PROGRAM void Intersect_TriangleMesh(int primIdx)
{
	...
	if (rtPotentialIntersection(t))
	{
		...
		dgShading.dpdu = dpdu;
		dgShading.dpdv = dpdv;
		dgShading.nn = nGeometry = make_float4(TwUtil::safe_normalize(cross(dp1, dp2)));
        ...
        if (normalBuffer.size())
		{
			...
			const int3 vn_idx = normalIndexBuffer[primIdx];
			ns = b0 * normalBuffer[vn_idx.x] + b1 * normalBuffer[vn_idx.y] + b2 * normalBuffer[vn_idx.z];
			ns = TwUtil::safe_normalize(ns);
