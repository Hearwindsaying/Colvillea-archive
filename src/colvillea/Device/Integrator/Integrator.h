...
#pragma endregion REGION_YLMCoeff_MUL
    ...
    static __device__ __inline__ void SHEvalFast15(const float3 &w, float *pOut)
    {
        const float fX = w.x;
        const float fY = w.y;
        const float fZ = w.z;
        ...
        if (!Cl::CheckOrientation<3>(quadShape))
        {
            ...
            if (clippedQuadVertices != 0)
            {
                ...
                else if (shaderParams.bsdfType == BSDFType::Plastic)
                {
                    ...
                    Cl::SHEvalFast15(wo_Local, ylmVector);
                    ...
                    //    }
                    //}
                    //if (sysLaunch_index == make_uint2(729, 720 - 545)|| sysLaunch_index == make_uint2(729, 720 - 562)||
                    //    sysLaunch_index == make_uint2(729, 720 - 563))
                    //{
                    ...
                    for (int i = 0; i < (lmax + 1)*(lmax + 1); ++i)
                    {
                        /*if (sysLaunch_index == make_uint2(729, 720 - 545) || sysLaunch_index == make_uint2(729, 720 - 562) ||
                            sysLaunch_index == make_uint2(729, 720 - 563))
                        {
                            rtPrintf("%d] L:%f += FlmVector[%d]:%f ylmCoeff[%d]:%f\n", 
                                sysLaunch_index.y, L.x,     i, FlmVector[i], i, ylmCoeff[i]);
                        }*/
                        //if (i == a_n)
                        //{
                        //    //
